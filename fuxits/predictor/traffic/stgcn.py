...
class STConv(nn.Module):
    def __init__(self, cheb_k, time_cov_kernel, hidden_size, num_nodes, static_adj, **kwargs) -> None:
...
class STGCN(Predictor):
    def __init__(self, config, in_channels, num_nodes, hist_steps, pred_steps, static_adj):
        ...
        self.st_conv1 = STConv(cheb_k, time_cov_kernel, hidden_size[:3], num_nodes, chebpoly)
        self.st_conv2 = STConv(cheb_k, time_cov_kernel, hidden_size[2:], num_nodes, chebpoly)
        self.dropout = nn.Dropout(self.model_config['drop_ratio'])
        self.tconv1 = Temporal_Conv(hidden_size[-1], hidden_size[-1], hist_steps - 4 * (time_cov_kernel - 1), "glu")
        self.ln = nn.LayerNorm([num_nodes, hidden_size[-1]])
        self.tconv2 = Temporal_Conv(hidden_size[-1], hidden_size[-1], 1, "sigmoid")
        self.fc = nn.Conv2d(hidden_size[-1], pred_steps, 1)

    def forward(self, x):
        x_st1 = self.st_conv1(x)
        x_st2 = self.st_conv2(x_st1)
        x_t1 = self.tconv1(x_st2)
        x_ln = self.ln(x_t1.transpose(1,3)).transpose(1, 3) # b x f x n x 1
        x_t2 = self.tconv2(x_ln)
        return self.fc(x_t2).transpose(1, 3) # b x t x n x f
