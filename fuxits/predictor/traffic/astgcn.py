...
class ASTGCN_Sub(nn.Module):
    ...
    def forward(self, x):
        ...
        for m in self.modulelist:
            x = m(x)
        return self.conv(x.transpose(1, 3)).transpose(1, 3) # B x F x N x T -> B x T x N x F -> B x T_out x N x F_in -> B x F_in x N x T_out
...
class ASTGCN_Block(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        x1 = self.TAt(x)
        x2 = F.relu(self.cheb_conv(x, self.SAt(x1)))
