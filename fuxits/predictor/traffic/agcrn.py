...
class AGCRNCell(nn.Module):
    ...
    def forward(self, x, state, node_embeddings):
        ...
        t = self.update(candidate, node_embeddings)
        hc = torch.tanh(t)
        h = r*state + (1-r)*hc
        return h
...
class AGCRN(Predictor):
    def __init__(self, config, in_channels, num_nodes, hist_steps, pred_steps, static_adj, **kwargs):
        ...
        self.encoder = AVWDCRNN(self.num_nodes, self.input_dim, hidden_size, cheb_k, embed_dim, num_layers)

        #predictor
        self.end_conv = nn.Conv2d(1, self.pred_steps * self.output_dim, kernel_size=(1, hidden_size), bias=True)
    ...
    def forward(self, x, y=None, batch_idx=None):
        ...
        output = output.squeeze(-1).reshape(-1, self.pred_steps, self.output_dim, self.num_nodes)
